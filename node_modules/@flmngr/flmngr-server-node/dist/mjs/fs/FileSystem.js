...
export class FileSystem {
    ...
    getRelativePath(path) {
        if (path.indexOf("..") > -1) {
            throw new MessageException(Message.createMessage(false, Message.FM_DIR_NAME_CONTAINS_INVALID_SYMBOLS));
        }
        if (path.indexOf("/") !== 0) {
            path = "/" + path;
        }
        ...
        if (path.indexOf("/" + rootDirName) !== 0) {
            throw new MessageException(Message.createMessage(false, Message.FM_DIR_NAME_INCORRECT_ROOT));
        }
    ...
    reqGetDirs(request) {
        ...
        for (const dirStr of dirsStr) {
            ...
            dirs.push(new FMDir(dirArr[dirArr.length - 1], dirArr.slice(0, dirArr.length - 1).join("/"), filled).getJSON());
        }
        return dirs;
    }
    reqGetFilesPaged(request) {
        ...
        if (maxFiles < 1) {
            throw new MessageException(Message.createMessage(false, Message.MALFORMED_REQUEST));
        }
        ...
        files = files.sort((arr1, arr2) => {
            for (let i = 0; i < 3; i++) {
                if (typeof arr1[i] === "string") {
                    let v = Utils.strnatcmp(arr1[i], arr2[i]);
                    if (v !== 0) {
                        return v;
                    }
                }
    ...
    getCachedFile(filePath) {
        return new CachedFile(filePath, this.driverFiles, this.driverCache, this.onLogError);
    }
    ...
    reqResizeFile2(request) {
        return __awaiter(this, void 0, void 0, function* () {
            ...
            if (mode === 'IF_EXISTS' && !isDstPathExists) {
                throw new MessageException(Message.createMessage(false, Message.FM_NOT_ERROR_NOT_NEEDED_TO_UPDATE));
            }
    ...
    reqGetImageOriginal(request) {
        ...
        if (mimeType == null) {
            throw new MessageException(Message.createMessage(false, Message.FM_FILE_IS_NOT_IMAGE));
        }
    ...
    reqUpload(request) {
        ...
        if (!file) {
            throw new MessageException(Message.createMessage(false, Message.FILES_NOT_SET));
        }
